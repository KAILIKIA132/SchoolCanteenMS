...
# - Compiles all src Java sources (Java 8 target)
...
function Write-Ok([string]$msg) {
    Write-Host ("[{0}] {1}" -f (Get-Date).ToString("HH:mm:ss"), $msg) -ForegroundColor Green
}

function Write-Fail([string]$msg) {
    Write-Host ("[{0}] {1}" -f (Get-Date).ToString("HH:mm:ss"), $msg) -ForegroundColor Red
}
...
if (-not $SkipWar) {
    ...
    if (-not $jarCmd) {
        ...
        Write-Ok ("WAR created: " + $wi.FullName + " (" + $mb + " MB)")
    }
} else {
